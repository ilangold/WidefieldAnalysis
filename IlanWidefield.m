function varargout = IlanWidefield(varargin)

%%%%%%%% Setup environment %%%%%%%%
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
    'gui_Singleton',  gui_Singleton, ...
    'gui_OpeningFcn', @IlanWidefield_OpeningFcn, ...
    'gui_OutputFcn',  @IlanWidefield_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
addpath(genpath(['C:\Ilan_Psignal\IlanWidefield']));

function IlanWidefield_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = IlanWidefield_OutputFcn(hObject, eventdata, handles)
...
%animalname name
bb = strsep(PsignalfileName,'_');
set(handles.animalID,'String',bb{2});
guidata(hObject, handles)
...
psignaldata = LoadPsignalParams(handles.Psignalfile);
...
t=1;
keystim=[];
AttenRange = -1*psignaldata.Primary.AttenRange;
for i = 1:length(exptevents)
    strparts = strsep(exptevents(i).Note,',',1);
    if strcmpi(deblank(strparts{1}),'Stim')
        if sum(AttenRange) > 0
            AttendB=-20*log10((str2num(strparts{4})))
            FreqLevelOrder = [FreqLevelOrder; str2num(strparts{2}) OveralldB-AttendB];
        else
...
%Noise or tones
NoiseOrTones = psignaldata.Primary.Type;
handles.noise = 0;
...
handles.pfs=pfs;
handles.PrimaryDuration = psignaldata.Primary.Duration;
handles.PreStimSilence = psignaldata.Primary.PreStimSilence;
handles.PostStimSilence = psignaldata.Primary.PostStimSilence;
...
if isempty(strfind(handles.surfacefile,'tif'))
    ...
    TrialsPerFreq = psignaldata.TrialObject.TrialsPerFreq;
    ...
    %Take off edges of images, and only keep expected number of images.
    reps = str2num(get(handles.reps,'String'));
    if reps > 0
        TrialsPerFreq = reps;
    else
        if isfield(psignaldata.TrialObject,'TrialsPerFreq')
            TrialsPerFreq = psignaldata.TrialObject.TrialsPerFreq;
        else
...
DSTP = imresize(Tonotopy,0.5);
handles.FreqKept = unique(round(Tonotopy(~isnan(Tonotopy))));
Freqidx = unique(round(DSTP(~isnan(DSTP))));
outputFreqs = handles.Freqs;
handles.FreqKept = unique(round(Tonotopy(~isnan(Tonotopy))));
...
surfaceImg = imresize(handles.surfaceAP,0.5);
surfaceMask = imresize(handles.mask,0.5);
tonotopicMap = imresize(TonotopicMap,0.5);
...
% freezeColors
hold off
outputFileName = 'TonotopyOutput.mat';
outputFile = strcat(handles.surfaceFilePath,outputFileName);
save(outputFile,'DSTP','Freqidx','outputFreqs','surfaceImg','surfaceMask','tonotopicMap');
guidata(hObject, handles)
...
if strcmpi(analysis,'Time-course')
    %Time-course
    [a b c d e] = size(handles.DeltaFF);
    DeltaFFAP = zeros(b,a,c,d,e);
    % DeltaFFAP = zeros(handles.DeltaFF);
    ...
    interpfact=2;
    pfs=handles.pfs*interpfact;
    f=linspace(handles.Freqs(1), handles.Freqs(end),interpfact*length(handles.Freqs));
    if size(PointDeltaFFdBTime,2) > 1
        InterpTC = interp2(PointDeltaFFdBTime, interpfact);
        t=-handles.PreStimSilence:1/(pfs):(size(InterpTC,1)/pfs)-(1/pfs)-handles.PreStimSilence;
        ...
        InterpTC = interp(PointDeltaFFdBTime, interpfact);
        t=-handles.PreStimSilence:1/(pfs):(size(InterpTC,1)/pfs)-(1/pfs)-handles.PreStimSilence
        plot(t,InterpTC.*100,'k','Parent',handles.analysisfig);
        hold on
        aa=axis;
        hold on
        plot([0 0],[aa(3) aa(4)],'k:','linewidth',2)
        ylabel('\DeltaF (%)')
    end
