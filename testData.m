...
tarTrace = squeeze(nanmean(nanmean(nanmean(PassMat(:,:,:,Freqind(:,2,3)),1),2),4));
nonTrace = squeeze(nanmean(nanmean(nanmean(PassMat(:,:,:,Freqind(:,2,6)),1),2),4));
target = (nanmean(nanmean(PassMat(:,:,:,Freqind(:,2,3)),3),4));
nontarget = (nanmean(nanmean(PassMat(:,:,:,Freqind(:,2,6)),3),4));
...
for i = 1:length(pixCoords)
    ...
    for ii = 1:pr
        mutarTrace{i,1}(ii,1) = abs(nanmean(nanmean(squeeze(PassMat(pixCoords{i,1}(ii,1),pixCoords{i,1}(ii,2),idx,Freqind(:,2,3))),2)));
        munonTrace{i,1}(ii,1) = abs(nanmean(nanmean(squeeze(PassMat(pixCoords{i,1}(ii,1),pixCoords{i,1}(ii,2),idx,Freqind(:,2,6))),2)));
    end
end
...
save('C:\Users\PsiDev\Desktop\WF_data\WF_Behavior\test\testData','CR','DeltaFFds','DSTP',...
    'F','Freqidx','Freqind','H','M','outputFreqs','pixCoords','target','nontarget',...
    'tarTrace','nonTrace','mutarTrace','munonTrace')