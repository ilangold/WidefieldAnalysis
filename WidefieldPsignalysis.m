...
t=1;
keystim=[];
AttenRange = psignaldata.Primary.AttenRange;
for i = 1:length(exptevents)
    ...
    if strcmpi(deblank(strparts{1}),'Stim')
        ...
        if strcmpi(exptparams.runclass,'AHL') || strcmpi(exptparams.runclass,'RND')
...
if isempty(strfind(handles.surfacefile,'tif'))
    ...
    try
        I = I(framepix:end-framepix-1, :, ...
            1:length(unique(stimypes))*framespertrial*TrialsPerFreq*length(handles.Freqs)*length(handles.Levels));
    catch
...
I = reshape(I,[size(I,1) ...
    size(I,2) ...
    framespertrial ...
    length(unique(stimypes))*TrialsPerFreq*length(handles.Freqs)*length(handles.Levels)]);
...
for i = 1:size(handles.muDeltaFdBAP,1)
    for ii = 1:size(handles.muDeltaFdBAP,2)
        ...
        [junk f]=max(abs(squeeze(handles.muDeltaFdBAP(i,ii,f))));
